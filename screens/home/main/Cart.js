...
const Cart = ({ navigation }) => {
  ...
  const LeftContent = props => <Avatar.Icon {...props} icon="folder-information-outline" />
  ...
  const fetchData = async () => {
    ...
    await axios.get(`${API}/data/cart/${route.params.user._id}`)
    .then(res => {
      setData(res.data.cart); 
      setBackup(res.data.cart);
    });
  }
    ...
    <ScrollView contentContainerStyle={{ 
      ...
      >
        <Searchbar
          ...
          value={search}
        />
        <Text medium color="#06bcee" style={{marginBottom:10}}>Toutes les demandes {data.length}</Text>
        {
          data.map((item, index) => (
