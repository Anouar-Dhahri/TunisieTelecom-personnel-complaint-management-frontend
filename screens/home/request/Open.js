...
const Open = ({ navigation }) => {
  ...
  const LeftContent = props => <Avatar.Icon {...props} icon="camera-timer" />
  ...
  const fetchData = async () => {
    ...
    await axios.get(`${API}/data/request/open/${route.params.user._id}`)
    .then(res => {
      setData(res.data.open); 
      setBackup(res.data.open)
    });
  }
    ...
    <ScrollView contentContainerStyle={{ 
      ...
      >
        <Searchbar
          ...
          value={search}
        />
        <Text medium color="#06bcee" style={{marginBottom:10}}>Toutes les demandes ouvertes {data.length}</Text>
