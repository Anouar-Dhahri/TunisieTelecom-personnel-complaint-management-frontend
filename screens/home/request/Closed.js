...
const Closed = ({ navigation }) => {
  ...
  const LeftContent = props => <Avatar.Icon {...props} icon="lock-outline"/>
  ...
  const fetchData = async () => {
    ...
    await axios.get(`${API}/data/request/close/${route.params.user._id}`)
    .then(res => {
      setData(res.data.close); 
      setBackup(res.data.close);
    });
  }
    ...
    <ScrollView contentContainerStyle={{ 
      ...
      >
        <Searchbar
          ...
          value={search}
        />
        <Text medium color="#06bcee" style={{marginBottom:10}}>Toutes les demandes fermées {data.length}</Text>
